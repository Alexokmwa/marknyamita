...
<div>
	<nav class="<?=$this->nav_class?>"
		style="<?=$this->nav_styles?>">
		<ul class="<?=$this->ul_class?>"
			style="<?=$this->ul_styles?>">
			<li class="<?=$this->first_li_class?>"
				style="<?=$this->first_li_styles?>"><a
					class="<?=$this->first_a_class?>"
					style="<?=$this->first_a_styles?>"
					href="<?=$this->links['first']?>">First</a>
			</li>

			<?php for($x = $this->start; $x <= $this->end;$x++):?>
			<li style="<?=$this->li_styles?>;<?=($x == $this->page_number) ? $this->active_styles : '';?>"
				class="<?=$this->li_class?> 
 			    	<?=($x == $this->page_number) ? $this->active_class : '';?>
 			    	"><a style="<?=$this->a_styles?>"
					class="<?=$this->a_class?>" href="
 			    		<?= preg_replace('/page=[0-9]+/', "page=".$x, $this->links['current'])?>
 			    		"><?=$x?></a></li>
			<?php endfor;?>

			<li class="<?=$this->next_li_class?>"
				style="<?=$this->next_li_styles?>"><a
					style="<?=$this->next_a_styles?>"
					class="<?=$this->next_a_class?>"
