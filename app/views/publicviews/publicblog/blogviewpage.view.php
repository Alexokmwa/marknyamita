...
<section>
	<div class="container">
        ...
        <a href="#" class="badge text-bg-danger mb-2"><i class="fas fa-circle me-2 small fw-bold"></i><?=$rowpost->category?></a>
				<h1 class="display-4"><?=$rowpost->postname?></h1>
        <p class="lead"><?=$rowpost->shortdescription?></p>
        <!-- Info -->
        <ul class="nav nav-divider align-items-center">
          ...
          <?php if (is_array($data['rowcreator']) && count($data['rowcreator'])): ?>
            ...
            <div class="nav-link">
              by <a href="#" class="text-reset btn-link"><?= esc($rowcreator->firstname . " " . $rowcreator->lastname) ?></a>
            </div>
            <?php endif; ?>
													<?php endforeach; ?>
													<?php endif; ?>
          </li>
          <li class="nav-item"><?= date('M d, Y', strtotime($rowpost->createdAt)); ?></li>
        ...
        <?php
                    $imageSrc = ROOTADMIN . esc($rowpost->imageurl);
						    ...
						    ?>
        <img class="rounded mt-5 img-fluid" src="<?=$imageSrc?>" alt="Image" style="object-fit: contain;width: 100%; height: auto;">
      </div>
    </div>
	</div>
</section>
...
<section class="pt-0">
	<div class="container position-relative">
		<div class="row">
      	...
      	<p><span class="dropcap bg-primary bg-opacity-10 text-primary px-2">R</span><?=$rowpost->postbody?> 
