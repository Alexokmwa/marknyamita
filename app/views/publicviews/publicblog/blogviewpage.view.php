...
<?php if ($successMessage): ?>
    <div class="position-fixed  top-0 end-0 p-3" style="z-index: 1050;">
        <div id="toastSuccess" class="toast align-items-center" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="true" data-bs-delay="5000">
...
<?php if (is_array($data['comment']) && count($data['comment'])): ?>
    ...
    <!-- Bootstrap Modal -->
    <div class="modal fade" id="imageModal-<?= $comment->commentID ?>" tabindex="-1" aria-labelledby="imageModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-fullscreen">
            <div class="modal-content">
                ...
                <div class="modal-body d-flex justify-content-center align-items-center">
                    <img src="<?= $imageSrclogged ?>" class="img-fluid" alt="comment photo">
                </div>
            </div>
        </div>
    </div>
<?php endif; ?>
                            <h5 class="m-0"><?= htmlspecialchars($rowcreatorusers->firstname . " " . $rowcreatorusers->lastname) ?></h5>
                        ...
                        <div class="collapse" id="replyForm-<?= $comment->commentID ?>">
                            ...
                            <?php else: ?>
                                <form id="addBlogCommentReplyForm" method="post" enctype="multipart/form-data">
                                    <input type="hidden" name="form_type" value="add_blog_comment_reply_not_logged_in_fromloggeduser">
                                    <input type="hidden" name="commentID" value="<?= $comment->commentID ?>">
                                    <div class="col-md-6">
                                        <label class="form-label">Name *</label>
                                        <input name="namereply" type="text" class="form-control">
...
<?php if (is_array($data['commentreply']) && count($data['commentreply'])): ?>
    <div class="collapse" id="replies-<?= $comment->commentID ?>">
        <?php foreach ($data['commentreply'] as $commentreply): ?>
            <?php if ($commentreply->commentID === $comment->commentID): ?>
                <div class="my-4 d-flex ps-2 ps-md-3">
                    ...
                    <div>
                        <p><?= htmlspecialchars($commentreply->Contentreply, ENT_QUOTES, 'UTF-8') ?></p>
                        <?php if (!empty($commentreply->usercommentimage)): ?>
                            <div class="col-12">
                                ...
                                <img class="rounded"
                                    ...
                                    style="object-fit: cover; height:150px; width:500px;"
                                    data-bs-toggle="modal"
                                    data-bs-target="#imageModalreply-<?= $commentreply->commentID ?>"> <!-- Ensure unique ID -->
                            </div>

                            <!-- Bootstrap Modal -->
                            <div class="modal fade" id="imageModalreply-<?= $commentreply->commentID ?>" tabindex="-1" aria-labelledby="imageModalLabelreply-<?= $commentreply->commentID ?>" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-fullscreen">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="imageModalLabelreply-<?= $commentreply->commentID ?>">Image</h5> <!-- Unique ID for each modal -->
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body d-flex justify-content-center align-items-center">
                                            <img src="<?= $imageSrcreply ?>" class="img-fluid" alt="comment photo">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<?php if (!empty($commentnotlogged->usercommentimage)): ?>
    ...
    <!-- Bootstrap Modal -->
    <div class="modal fade" id="imageModal-<?= $commentnotlogged->commentID ?>" tabindex="-1" aria-labelledby="imageModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-fullscreen">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="imageModalLabel">Image</h5>
                ...
                <div class="modal-body d-flex justify-content-center align-items-center">
                    <img src="<?= $imageSrc ?>" class="img-fluid" alt="comment photo">
                </div>
            </div>
        </div>
    </div>
<?php endif; ?>
						...
						<?php if ($ses->isLoggedIn()): ?>
								...
								<input type="hidden" name="form_type" value="add_blog_comment_reply_comment_from_notlogged">
										<div class="col-md-6">
											<label class="form-label">Name *</label>
											<input name="namereply" type="text" class="form-control">
...
<?php if (is_array($data['commentreplynotlogged']) && count($data['commentreplynotlogged'])): ?>
    <div class="collapse" id="replies-<?= $commentnotlogged->commentID ?>">
            ...
            <?php if ($commentreplynotlogged->commentID === $commentnotlogged->commentID): ?>
                ...
                <div class="my-4 d-flex ps-2 ps-md-3">
                    <img class="avatar avatar-md rounded-circle float-start me-3"
                        src="assets/images/avatar/02.html" alt="avatar">
                    <div>
                        <p><?= htmlspecialchars($commentreplynotlogged->Contentreply, ENT_QUOTES, 'UTF-8') ?></p>
                        <?php if (!empty($commentreplynotlogged->usercommentimage)): ?>
                            <div class="col-12">
                                ...
                                <img class="rounded"
                                     ...
                                     style="object-fit: cover; height:150px; width:500px;"
                                     data-bs-toggle="modal"
                                     data-bs-target="#imageModalreplynotlogged-<?= $commentreplynotlogged->replyID ?>"> <!-- Unique ID for each image -->
                            </div>

                            <!-- Bootstrap Modal -->
                            <div class="modal fade" id="imageModalreplynotlogged-<?= $commentreplynotlogged->replyID ?>" tabindex="-1" aria-labelledby="imageModalLabel-<?= $commentreplynotlogged->replyID ?>" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-fullscreen">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="imageModalLabel-<?= $commentreplynotlogged->replyID ?>">Image</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body d-flex justify-content-center align-items-center">
                                            <img src="<?= $imageSrcnotloggedreply ?>" class="img-fluid" alt="comment reply photo">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?>
                        <div class="mb-2">
                            <h5 class="m-0"><?= htmlspecialchars($commentreplynotlogged->namereply, ENT_QUOTES, 'UTF-8') ?></h5>
                            <span class="me-3 small"><?= date('F j, Y \a\t g:i a', strtotime($commentreplynotlogged->CreatedAt)) ?></span>
                        </div>
                    </div>
                </div>
                <!-- Comment children level 2 end -->
...
<div>
    ...
    <input type="hidden" name="form_type" value="add_blog_comment_not_logged_in">
        ...
        <p class="text-danger">
            You are currently not logged in. You may not have control over this comment in the future after you terminate this session. Please <a href="<?=ROOT?>Login">log in</a> to have lifetime control of this comment.
        </p>
        <div class="col-12">
            <button type="submit" class="btn btn-primary">Post comment</button>
        </div>
