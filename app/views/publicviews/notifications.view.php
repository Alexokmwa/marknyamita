...
<main class="mysmbottomspace">
	<div class="card-header bg-transparent border-bottom p-3 d-flex justify-content-between align-items-center">
		<?php $num = getnotificationspublic();$numevent = eventgetnotificationspublic()  ?>
		<?php if ($num || $numevent): ?>
		<?php $eventnum =$num +$numevent ?>
		<h6 class="m-0">Notifications <span
				class="badge bg-danger bg-opacity-10 text-danger ms-2"><?= $eventnum ?>
				new</span></h6>
		<?php endif ?>
	</div>
...
<a href="<?= ROOT . "Blogview/" . $row->Itemid . "?seen=1&notif=" . $row->id?>" class="d-flex align-items-center text-decoration-none w-100" style="background-color:inherit;">
    <div class="me-3">
...
<?php endif?>
		...
		<?php endif; ?>
		<?php if (!empty($rowspublicevent)): ?>
		<ul class="list-group list-unstyled list-group-flush">
			<?php foreach ($rowspublicevent as $row): ?>
			<?php
                        $color = $row->seen ? "white" : "rgb(185, 232, 185)";
			    $text = $row->seen ? "read" : "new notification";
			    $colortext = $row->seen ? "red" : "blue";
			...
			<li class="d-flex align-items-center text-decoration-none pt-2 mb-3"
				style="background-color: <?= $color ?>">
	<a href="<?= ROOT . "Sigleevent/" . $row->Itemid . "?seen=1&notifevent=" . $row->id;?>" class="d-flex align-items-center text-decoration-none w-100" style="background-color:inherit;">
    <div class="me-3">
        <div class="avatar avatar-sm">
            <img class="avatar-img rounded-circle"
                src="<?= isset($row->user_row) && $row->user_row ? get_imageadmin($row->user_row->adminimage) : '' ?>"
    ...
    <div class="flex-grow-1">
        <h6 class="mb-1 text-center">
            ...
            <?php
                switch ($row->type) {
                   case 'eventpost':
                        echo 'created an event<br/><i>' . esc($row->item_rowevent->eventname) . '</i>';
                        break;
                    default:
                        // handle other cases if necessary
                        break;
    ...
    <div class="d-flex align-items-center ms-3">
        <div class="me-3">
            <div class="avatar avatar-sm">
                <?php $postpic = ROOTADMIN . esc($row->item_rowevent->eventimage); ?>
                <img class="avatar-img rounded-circle"
                    src="<?= $postpic ?>"
                    alt="avatar">
            </div>
				...
				<?php if($row->seen == 1):?>
				<form id="deletenotif" method="post" >
				<input type="hidden" name="form_type" value="eventdelete_notification">
				<input type="hidden" name="id" value="<?= $row->id ?>">
				<button type="submit" class="btn btn-danger" >clear</button>
				</form>
...
<?php endif?>
		...
		<?php endif; ?>
		<?php if (empty($rowspublic) && empty($rowspublicevent)): ?>
